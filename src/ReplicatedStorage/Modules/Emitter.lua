...
local signalModule = require(Modules:WaitForChild("Signal")) -- Feel free to change to whatever signal module you have
...
function emitterMethods.SetEmitterRate(self: ParticleEmitter2D, NEW_RATE: number): ParticleEmitter2D
	...
	if math.sign(NEW_RATE) == -1 then
        error('Rate can not be negative!')
    end

	if NEW_RATE >= 100 then
        warn('Setting the emission rate to high values might cause performance issues.')
    end
...
function emitterMethods.SetPropertyTransition(self: ParticleEmitter2D, PROPERTY_NAME: string, TRANSITION_SEQ: ColorSequence | NumberSequence): ParticleEmitter2D
	...
	if self.TemplateParticle == nil then
        error('Cannot set value! Template particle must be set first.')
    end

	local isColorSequence = typeof(TRANSITION_SEQ) == 'ColorSequence'
	local isNumberSequence= typeof(TRANSITION_SEQ) == 'NumberSequence'

	if not isColorSequence and not isNumberSequence then
        error('Argument TRANSITION_SEQ must be a ColorSequence or NumberSequence. Got '..typeof(TRANSITION_SEQ))
    end

	local propertyTransition: any, _ = self:GetPropertyTransition(PROPERTY_NAME)
	...
	if propertyTransition == nil then
		...
		if not success then
            error(`Could not find property {PROPERTY_NAME} for {self.TemplateParticle.ClassName}!`)
        end
...
function emitterMethods.RemovePropertyTransition(self: ParticleEmitter2D, PROPERTY_NAME: string): ParticleEmitter2D
	...
	if not index then
        error(`Could not find {PROPERTY_NAME} in property transitions array!`)
    end
...
function particleMethods.Destroy(self: Particle2D)
	...
	table.remove(self.EmittedBy.Particles, index)
...
function emitYourParticleModule.newParticle(EMITTER: ParticleEmitter2D): Particle2D
	if not EMITTER.TemplateParticle then
        error('Template particle must be set before emitting a particle!')
    end
