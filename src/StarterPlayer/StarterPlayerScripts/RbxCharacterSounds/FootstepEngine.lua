...
function FootstepEngine.PlayFootstep(Character: Model)
	local data = CharacterData[Character]
	if not data then 
		warn("Character not initialized for footsteps:", Character.Name)
		return 
	end
	...
	if sound then
		...
		warn("No footstep sound found for material:", materialName)
	end
end
