...
local FootstepEngine = require(script:WaitForChild("FootstepEngine"))
...
local function initializeSoundSystem(instances: { [string]: Instance })
	...
	-- Footstep attribute handler
	local footplantedConnection
	footplantedConnection = character.AttributeChanged:Connect(function(AttributeName: string)
		if AttributeName == "Footplanted" then
			local Value = character:GetAttribute(AttributeName)
			if Value then
				stopPlayingLoopedSounds()
				-- Use the FootstepEngine instead of the character sound system
	...
	local stateTransitions: {[Enum.HumanoidStateType]: () -> ()} = {
		...
		-- Removed Running state transition
	...
	-- updaters for looped sounds (removed Running updater)
	...
	local function terminate()
		...
		-- Disconnect footplanted connection
		if footplantedConnection then
			footplantedConnection:Disconnect()
		end
